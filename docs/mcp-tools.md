...
REPLY: a PNG image plus a metadata text block. With writeToFile: true there is NO image — the reply is one JSON object of that same metadata with `filePath` added, naming an absolute path on the machine the server runs on.
